...
export default function AdminLogin() {
  ...
  const stepConfig = {
    email:       { icon: <Mail className="w-5 h-5 text-blue-400" />,    title: "Forgot Password",    desc: "Enter your admin email to receive a 6-digit OTP." },
    otp:         { icon: <ShieldCheck className="w-5 h-5 text-blue-400" />, title: "Enter OTP",        desc: `An OTP was sent to ${forgotEmail}. It expires in 10 minutes.` },
    newPassword: { icon: <KeyRound className="w-5 h-5 text-blue-400" />, title: "Set New Password",   desc: "Choose a strong new password for your account." },
  };
              ...
              <Card className="w-full max-w-md bg-slate-900 border-blue-500/20 shadow-2xl">
                ...
                <form onSubmit={handleLogin}>
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <button
                        ...
                        className="text-xs text-blue-400 hover:text-blue-300 transition-colors mt-1 float-right"
                      >
                        Forgot Password?
                      </button>
                    </div>
                  ...
                  <CardFooter className="flex-col gap-4 pt-6">
                    ...
                    <ProgressBar isLoading={isLoading} color="bg-blue-500" />
                ...
                <CardHeader>
                  ...
                  <div className="flex gap-2 mt-4">
                    {(["email", "otp", "newPassword"] as ForgotStep[]).map((s, i) => (
                      <div key={s} className={`h-1 flex-1 rounded-full transition-all ${forgotStep === s || (i < ["email","otp","newPassword"].indexOf(forgotStep)) ? "bg-blue-500" : "bg-slate-700"}`} />
                    ))}
                  </div>
                </CardHeader>
                ...
                <AnimatePresence mode="wait">
                  {forgotStep === "email" && (
                    <motion.form key="step-email" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleRequestOtp}>
                      ...
                      <CardFooter className="flex-col gap-3">
                        <Button type="submit" disabled={isForgotLoading} className="w-full bg-blue-600 hover:bg-blue-500 text-white">
                          {isForgotLoading ? "Sending OTP..." : "Send OTP"}
                        </Button>
                        <ProgressBar isLoading={isForgotLoading} color="bg-blue-500" />
                  ...
                  {forgotStep === "otp" && (
                    <motion.form key="step-otp" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleVerifyOtp}>
                      ...
                      <CardFooter className="flex-col gap-3">
                        <Button type="submit" disabled={isForgotLoading || otp.length !== 6} className="w-full bg-blue-600 hover:bg-blue-500 text-white">
                        ...
                        <div className="flex items-center justify-between w-full text-xs">
                          ...
                          {resendCountdown > 0 ? (
                            <span className="text-slate-500">
                              Resend in <span className="text-blue-400 font-mono font-bold">{resendCountdown}s</span>
                            </span>
                          ) : (
                            <button
                              type="button"
